...
function getSubscribers(channelId, subscriberChannelId, depth) {
    if (depth <= 2) {
        ...
        if (!channelsFound.includes(channelId)) {
            ...
            // subscriptions holen
            const subscriptions = httpGet("https://youtube.googleapis.com/youtube/v3/subscriptions?part=snippet&maxResults=50&channelId=" + channelId + "&key=" + apikey)
            try {
                subscriptions.items.forEach(subscription => {
                    getSubscribers(subscription.snippet.resourceId.channelId, channelId, depth + 1)
                });
            } catch { }
        }
    }
...
function saveChannelData(channelId, subscriberChannelId) {
    // daten holen
    const statistics = httpGet("https://youtube.googleapis.com/youtube/v3/channels?part=statistics&key=" + apikey + "&id=" + channelId).items[0].statistics;
    const snippet = httpGet("https://youtube.googleapis.com/youtube/v3/channels?part=snippet&key=" + apikey + "&id=" + channelId).items[0].snippet;
    ...
    con.query(`
            ...
            ('`+ channelId + `', '` + snippet.title + `', '` + statistics.viewCount + `', '` + statistics.subscriberCount + `', '` + statistics.videoCount + `', '` + snippet.defaultLanguage + `', '` + snippet.country + `')
            `, (err, result) => {
        if (err) throw err;
